...
//	error_reporting(E_ALL);
	...
	$r = mysqli_query($db,"SELECT * FROM bot,user WHERE bot.user=user.idUser AND nameBot='".$nameBot."'");
	...
	**************************************************/
	...
	curl_setopt($ch, CURLOPT_POSTFIELDS, "login=".$nameUser."&passlog=".$passUser."&hidden=log&logon=");
...
//		print($page_content);
	...
	$valeur = 600;
	curl_close($ch);
	if (preg_match('`(<b><small> \d+</small></b>)`i', $page_content,$matches)){
		$points = intval(substr($matches[1],11));
		echo "<h1> points : ".$points."</h1></br>";
	}
	else{
		echo "<H1>Pas de points</H1></br>";
	}
	if(preg_match("/Félicitation, ton vote/",$page_content)){
		$result = 1;
		$poins = $points + $pointParVote;
	}
	else{
		echo "nop";
		flush();
	...
	$query = "INSERT INTO vote (`date`,`bot`,`user`,`result`) VALUES ('".$today."','".$nameBot."','".$nameUser."','".$result."')";
	echo $query."</br>";
	...
	mysqli_query($db,"UPDATE user WHERE idUser='".$idUser."' SET points='".$points."'");
...
?>
>>>>>>> b822061d15fe77576b550458a8095838d095d41b
